...
def multi_scale_semantic_validate2(config, val_loader, val_dataset, model, criterion, output_dir,
             tb_log_dir, writer_dict=None, multi_scale_list=[1]):
    ...
    with torch.no_grad():
        ...
        for i, (inputs, targets, target_weights, metas) in enumerate(val_loader):
            ...
            for j in len(inputs):
                ...
                target = target[j].cuda(non_blocking=True)
...
def get_multi_scale_size(image, input_size, current_scale, min_scale):
    ...
    if w < h:
        ...
        scale_w = w / 256.0
        scale_h = h_resized / w_resized * w / 256.0
    else:
